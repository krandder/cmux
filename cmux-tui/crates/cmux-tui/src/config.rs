...
fn ghostty_defaults_from_helper() -> GhosttyHelperDefaults {
    ...
    let output_rx = read_ghostty_helper_output_async(stdout);
    ...
    match output_rx.recv_timeout(Duration::from_millis(50)) {
        ...
        Ok(None) | Err(_) => GhosttyHelperDefaults::Unavailable,
    }
}
...
fn read_ghostty_helper_output_async(
    ...
    let _ = std::thread::Builder::new().name("cmux-tui-ghostty-helper-output".to_string()).spawn(
        move || {
            let _ = tx.send(read_ghostty_limited_string(stdout, GHOSTTY_HELPER_OUTPUT_MAX_BYTES));
        },
    );
    rx
}
...
fn desktop_theme_command_output(program: &str, args: &[&str]) -> Option<String> {
    ...
    output_rx.recv_timeout(Duration::from_millis(10)).ok().flatten()
}
...
fn resolve_ghostty_theme_defaults(
    ...
    for candidate in theme_candidates {
        if let Some(defaults) =
            load_ghostty_theme(candidate, theme_dirs, theme_selection, overrides)
        {
            return defaults;
        }
    }
...
fn load_ghostty_theme(
    ...
    let ghostty_background = if theme_selection == GhosttyThemeSelection::Ghostty {
...
fn ghostty_theme_selection_background(
    ...
    overrides.bg.or_else(|| {
        let light = selected_conditional_ghostty_theme(
            value,
            GhosttyThemeSelection::Mode(GhosttyThemeMode::Light),
            None,
        )?;
...
mod tests {
    ...
    fn ghostty_config_helper_output_reader_drains_large_palette() {
        ...
        let rx = read_ghostty_helper_output_async(std::io::Cursor::new(output.clone()));

        assert_eq!(rx.recv_timeout(Duration::from_secs(1)).unwrap(), Some(output));
    }
    ...
    fn ghostty_config_helper_output_reader_enforces_byte_limit() {
        ...
        let rx = read_ghostty_helper_output_async(std::io::Cursor::new(output));
